...
option_list = list(
  ...
  make_option(c("--effect"),  type="character", default=NULL, help="Direct or population effects", metavar="character"),
...
snps <- data.table()
for (chr in 1:22) {
    clumps <- fread(paste0(opt$clump_prefix, ".chr", chr, ".clumped"))
...
if (opt$effect == "direct") {
    final %<>% 
        filter(direct_N == max(direct_N)) %>%
        ungroup()
} else if (opt$effect == "population") {
    final %<>% 
...
final %<>% 
    ...
    select("ID", "SNP", paste0(opt$effect, "_N"), paste0(opt$effect, "_SE"), "A1", "A2", "freq", paste0(opt$effect, "_Beta"), paste0(opt$effect, "_pval"), "Chr", "pos", "cptid") %>%
    setNames(c("ID", "Name", paste0(opt$effect, "_N"), paste0(opt$effect, "_SE"), "A1", "A2", "freq", "A1Effect", paste0(opt$effect, "_pval"), "Chrom", "Position", "varid"))


# save
fwrite(final, paste0("/var/genetics/proj/within_family/within_family_project/processed/clumping_analysis/", opt$pheno, "/", opt$effect, "/weights/", opt$dataset, "/meta_weights.snpRes.formatted"), quote = F, sep = "\t")    