...
if (phenotype == "migraine" | phenotype == "eczema") {
  ...
  ## 2 gen logistic linear mixed model fit with 5 PCs and NAGQ = 0
  glmm = glmer(phenotype ~ proband+paternal+maternal+(1|FID)+age+sex+agesex+V1+V2+V3+V4+V5,data=p,family=binomial(link='logit'),nAGQ=0,glmerControl(optimizer="bobyqa"))
  ...
  glmm = glmer(phenotype ~ proband+paternal+maternal+(1|FID)+age+sex+agesex+V1+V2+V3+V4+V5+V6+V7+V8+V9+V10,data=p,family=binomial(link='logit'),nAGQ=1,glmerControl(optimizer="bobyqa"))
  write.table(summary(glmm)$coefficients,paste0(outpath,'.2.effects.txt'),quote=F)
  write.table(as.matrix(vcov(glmm)),paste0(outpath,'.2.vcov.txt'),quote=F)
