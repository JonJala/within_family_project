...
## read in part ldsc results
data <- fread(paste0("/var/genetics/proj/within_family/within_family_project/processed/ldsc/partitioned_ldsc/", pheno, "/", effect, ".baselineLD.results"))
...
p1 <- ggplot(data) +
        ...
        geom_errorbar(aes(x=Category, ymin=Enrichment-1.96*Enrichment_std_error, ymax=Enrichment+1.96*Enrichment_std_error), width=0.2, colour="black", alpha=0.9, size=0.5) +
        coord_flip() +
        theme_classic() +
        theme(axis.title.y = element_blank())
ggsave(paste0("/var/genetics/proj/within_family/within_family_project/processed/ldsc/partitioned_ldsc/", pheno, "/", pheno, "_", effect, "_enrichment.png"), p1)

## prop h2 estimates
p2 <- ggplot(data) +
        geom_bar(aes(x=factor(Category, levels = rev(data$Category)), y=Prop._h2), stat="identity", fill="#0a4295", alpha=0.5) +
        geom_errorbar(aes(x=Category, ymin=Prop._h2-1.96*Prop._h2_std_error, ymax=Prop._h2+1.96*Prop._h2_std_error), width=0.2, colour="black", alpha=0.9, size=0.5) +
        coord_flip() +
        theme_classic() +
        theme(axis.title.y = element_blank())
ggsave(paste0("/var/genetics/proj/within_family/within_family_project/processed/ldsc/partitioned_ldsc/", pheno, "/", pheno, "_", effect, "_prop_h2.png"), p2)